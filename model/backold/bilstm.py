...
# @Last Modified time: 2018-03-08 13:09:37
import torch
...
class BiLSTM(nn.Module):
    def __init__(self, data):
        ...
        if self.char_all_feature:
            self.lstm = nn.LSTM(self.embedding_dim + self.char_hidden_dim*2, lstm_hidden, num_layers=self.lstm_layer, batch_first=True, bidirectional=self.bilstm_flag)
        else:
    ...
    def get_lstm_features(self, word_inputs, word_seq_lengths, char_inputs, char_seq_lengths, char_seq_recover):
        ...
        if self.use_char:
            ...
            if self.char_all_feature:
                ...
                ## concat word and char together
                word_embs = torch.cat([word_embs, char_features_extra], 2)
